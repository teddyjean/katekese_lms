...
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.batches.show', $material->batch_id) . '?tab=materi' }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-100 rounded-full px-4 py-1.5 transition-colors">&larr; Kembali</a>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-xl">
    <form method="POST" action="{{ route('admin.materials.update', $material) }}" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <p class="text-xs text-gray-500 mb-2">File saat ini: {{ $material->file_original_name ?? '(tidak ada)' }}</p>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Link Video <span class="text-gray-400">(opsional)</span></label>
            <input type="url" name="video_url" value="{{ old('video_url', $material->video_url) }}" placeholder="https://youtube.com/watch?v=..."
