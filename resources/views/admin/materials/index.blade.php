...
<div class="mb-6 flex items-start justify-between flex-wrap gap-y-3">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Materi</h1>
        <p class="text-gray-500 text-sm mt-1">Kelola materi pembelajaran per kelas</p>
    </div>
...
<form method="GET" class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-6 flex gap-3 flex-wrap">
    ...
    @if(request('batch_id'))
        <a href="{{ route('admin.materials.index') }}" class="text-sm text-gray-400 hover:text-red-500 px-3 py-2 transition-colors">Reset</a>
    @endif
</form>
    ...
    <table class="w-full min-w-[540px] text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
                ...
                <th class="text-left px-5 py-3.5 text-gray-500 font-semibold text-xs uppercase tracking-wide">File</th>
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            @empty
            <tr><td colspan="4" class="px-5 py-12 text-center text-gray-400">Belum ada materi.</td></tr>
            @endforelse
        </tbody>
    </table>
    </div>
    @if($materials->hasPages())
        <div class="px-5 py-4 border-t border-gray-100">{{ $materials->links() }}</div>
    @endif
</div>
@endsection
