...
<div class="mb-6 flex items-start justify-between flex-wrap gap-y-3">
    <div>
        <a href="{{ route('admin.batches.show', $material->batch_id) }}?tab=materi" class="inline-flex items-center gap-1.5 text-sm font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-100 rounded-full px-4 py-1.5 transition-colors">&larr; Kembali ke Materi</a>
        <h1 class="text-2xl font-bold text-gray-800 mt-2">{{ $material->title }}</h1>
        <p class="text-gray-500 text-sm mt-1">{{ $material->batch->name }}</p>
    </div>
    @if($canManage)
    <a href="{{ route('admin.materials.assessments.edit', $material) }}"
       class="shrink-0 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors shadow-sm">
        Penilaian Materi
    </a>
    @endif
</div>
...
{{-- Tugas & Test Terkait --}}
<div class="grid grid-cols-1 sm:grid-cols-2 gap-5 max-w-4xl mb-5">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <h2 class="font-semibold text-gray-700 mb-3">Tugas Terkait</h2>
        @forelse($material->assignments as $assignment)
            <a href="{{ route('admin.assignments.show', $assignment) }}"
               class="block text-sm text-gray-700 hover:text-blue-600 py-1.5 border-b border-gray-50 last:border-0">
                {{ $assignment->title }}
            </a>
        @empty
            <p class="text-sm text-gray-400">Belum ada tugas terkait materi ini.</p>
        @endforelse
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <h2 class="font-semibold text-gray-700 mb-3">Test Terkait</h2>
        @forelse($material->tests as $test)
            <a href="{{ route('admin.tests.show', $test) }}"
               class="block text-sm text-gray-700 hover:text-blue-600 py-1.5 border-b border-gray-50 last:border-0">
                {{ $test->title }}
            </a>
        @empty
            <p class="text-sm text-gray-400">Belum ada test terkait materi ini.</p>
        @endforelse
    </div>
</div>
