...
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.tests.show', $test) }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-100 rounded-full px-4 py-1.5 transition-colors">&larr; Kembali</a>
    <h1 class="text-2xl font-bold text-gray-800 mt-2">Edit Test</h1>
</div>
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-xl">
    <form method="POST" action="{{ route('admin.tests.update', $test) }}" class="space-y-4">
        ...
        <div>
            ...
            <select name="batch_id" id="batch_id" onchange="filterMaterials()" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300">
                @foreach($batches as $batch)
                    <option value="{{ $batch->id }}" @selected(old('batch_id', $test->batch_id) == $batch->id)>{{ $batch->name }}</option>
                @endforeach
            </select>
        </div>
        <div>
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $test->title) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
            <textarea name="description" rows="3" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300">{{ old('description', $test->description) }}</textarea>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Durasi (menit)</label>
            <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $test->duration_minutes) }}" min="1" max="300" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300">
        </div>
        <div class="pt-2 flex gap-3">
            ...
            <a href="{{ route('admin.tests.show', $test) }}" class="text-sm text-gray-500 hover:text-gray-700 px-3 py-2">Batal</a>
        </div>
    </form>
</div>
...
const savedMaterial = "{{ old('material_id', $test->material_id) }}";
